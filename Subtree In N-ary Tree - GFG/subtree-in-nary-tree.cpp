...
class Solution{
public:
  string postOrder(Node* node, unordered_map<string, int>& freq, vector<Node*>& result) {
    if (!node) return "";
    string subtree = "(";
    for (Node* child : node->children) {
        subtree += postOrder(child, freq, result);
    }
    subtree += to_string(node->data) + ")";
    freq[subtree]++;
    if (freq[subtree] == 2) {
